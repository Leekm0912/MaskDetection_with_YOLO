...
extern "C" {
    ...
    */
    ...
    // ----------------------------------------
    //사용자 정의 함수
    bool isPlaying = false; // 함수가 여러번 실행되는것을 막기위해
    bool isChecking = false; // 함수가 여러번 실행되는것을 막기위해
    int class1 = 0; // 마스크 O 측정
    int class2 = 0; // 마스크 X 측정(얼굴)
    double result = 0.0; // 시간 측정

    void checkTime(std::promise<int>* p, int misec) {
        isChecking = true; // 함수가 여러번 실행되는것을 막기위해
        // 1초동안 기다리는데 일단 이거는 수정가능
        // 1초동안 기다릴동안 다른 스레드에서 class1이랑 class2를 바꿀꺼임
        std::this_thread::sleep_for(std::chrono::milliseconds(misec));
        isChecking = false;
        int temp1 = class1;
        int temp2 = class2;
        class1 = 0; //초기화
        class2 = 0; //초기화
        p->set_value(temp1 < temp2 ? 2 : 1); // class2가 더 많이 실행됬으면 2, 아니면 1 리턴
    }

    void playWAV(int misec) {
        isChecking = true; // 함수가 여러번 실행되는것을 막기위해
        //std::promise<int> p; // 미래에 p에 값이 담길걸 약속하는 변수
        //std::future<int> data = p.get_future(); // 
        //thread t(checkTime, &p, misec); // checkTime을 스레드로 실행하고 매개변수로 p의 주소를 전달 -> 주소 안주니까 오류났음
        //t.join(); // 스레드가 끝날떄까지 기다려줌

        //data.wait(); // data에 값이 들어올때까지 기다림
        isChecking = false;
        //if (data.get() == 2) { // 만약 class2 가 더 많이 나왔다면
        isPlaying = true; // 돌림노래 방지
        printf("스레드재생중");
        sndPlaySoundA("C:\\Mask_X.wav", SND_ASYNC | SND_NODEFAULT); // 백슬래시는 2번 써줘야함(이스케이프 기호 때문)
        std::this_thread::sleep_for(std::chrono::seconds(3)); // 2초짜리 파일이라서 3초 멈췄음
        isPlaying = false;
        //}
    ...
    void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
    {
        try {
            ...
            //start = clock();
            //result = 0;
            int biggerSizeClassID = -1;
            int biggerSizeValue = 0; // 지금 함수는 상자 그릴때마다 호출되기 때문에 여기서 초기화 해줘도 됨.

            FILE* fp3 = fopen("__test3.txt", "a");
            //=======================
            cv::Mat *show_img = mat;
            int i, j;
            if (!show_img) return;
            ...
            for (i = 0; i < num; ++i) {
                ...
                for (j = 0; j < classes; ++j) {
                    ...
                    if (dets[i].prob[j] > thresh && show) {
                        ...
                        //수정
                        printf("class1 : %d class2 : %d result : %f\n", class1, class2, result);
                        if (class_id >= 0) {
                            box b = dets[i].bbox; // 아래에 내려보면 상자 그리는 알고리즘이 있고, 좌표도 구해줘서 이걸 응용했음
                            ...
                            int left = (b.x - b.w / 2.)*show_img->cols; // 왼쪽 좌표
                            int right = (b.x + b.w / 2.)*show_img->cols; // 오른쪽 좌표
                            int top = (b.y - b.h / 2.)*show_img->rows; // 윗쪽 좌표
                            int bot = (b.y + b.h / 2.)*show_img->rows; // 하단 좌표

                            if (left < 0) left = 0; // 기타 예외 발생시 처리해주는 if문들
                            if (right > show_img->cols - 1) right = show_img->cols - 1;
                            if (top < 0) top = 0;
                            if (bot > show_img->rows - 1) bot = show_img->rows - 1;

                            int width = right - left; // 가로길이 : 오른쪽좌표 - 왼쪽좌표
                            int height = bot - top; // 세로길이 : 아래좌표 - 위좌표
                            int size = width * height; // 넓이 : 가로 * 세로
                            if (size > biggerSizeValue) { // 만약 한번에 여러개가 발견되었을때, 새로 발견된사이즈가 이전에 저장한 값보다 크다면
                                biggerSizeValue = size; // 사이즈 저장하고
                                biggerSizeClassID = class_id; // 클래스 아이디 저장
                            }
                            printf("width : %d, height : %d\n", width, height);
                            printf("biggerSizeValue : %d, biggerSizeClassID : %d\n", biggerSizeValue, biggerSizeClassID);
                        }

                        if (dets[i].prob[j] >= thresh) {  // 정확도 설정
                            if (strcmp(names[j], "Mask") == 0) {
                                isMask = true;

                            }
                            else if (strcmp(names[j], "Human face") == 0) {
                                isMask = false;
                                ...
                                if (!isChecking) {
                                    printf("isChecking");
                                    if (!isPlaying) {
                                        thread t1(playWAV, 500); // 인식할 시간 misce
                                        t1.detach(); // 이건 join이랑 다르게 끝나는걸 안기다림 -> join쓰면 메인이 멈춰버려서 안됨
                                    }
                                }


                            }
                            if (biggerSizeClassID == 0) { // 저장한 클래스아이디가 사람얼굴(0번) 이라면
                                class2++;
                            }
                            else if (biggerSizeClassID == 1) { // 저장한 클래스아이디가 마스크(1번) 이라면
                                class1++;
                            }
                        ...
                        //수정===================
                        //fprintf(fp3,"\n%s: %.0f%% ", names[j], dets[i].prob[j] * 100);
                        //=======================
                    }
                }
                if (class_id >= 0) {
                    ...
                    if (copied_frame_id != frame_id) {
                        ...
                        if (copy_img == NULL) {
                            copy_img = cvCreateImage(cvSize(show_img->width, show_img->height), show_img->depth, show_img->nChannels);
                        }
            ...
            // Human face, Mask 카운트 출력 코드
            int maskCount = 0;
            int humanCount = 0;
            for (i = 0; i < num; ++i) {
                for (j = 0; j < classes; ++j) {
                    if (dets[i].prob[j] > thresh) {
                        if (strcmp(names[j], "Human face") == 0) {
                            humanCount++;
                        }
                        else if (strcmp(names[j], "Mask") == 0) {
                            maskCount++;
                        }
                    }
                }
            ...
            std::string str_mask;
            str_mask = "Mask : ";
            std::string str_no_mask;
            str_no_mask = "NO_Mask : ";

            std::string str_mask_count(std::to_string(maskCount));
            std::string str_no_mask_count(std::to_string(humanCount));
            int font_size = 1;
            cv::putText(*show_img, str_mask + str_mask_count, cv::Point(5, 40), cv::FONT_HERSHEY_COMPLEX_SMALL, 2 * font_size, CV_RGB(0, 255, 0), 4 * font_size, CV_AA);
            cv::putText(*show_img, str_no_mask + str_no_mask_count, cv::Point(5, 80), cv::FONT_HERSHEY_COMPLEX_SMALL, 2 * font_size, CV_RGB(255, 0, 0), 4 * font_size, CV_AA);
            // Human face, Mask 카운트 출력 코드
